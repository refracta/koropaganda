...
### `취업` 취업률 (`압도적`) `Y`위 (`Z`%)
`취업 * Y * Z%`를 올림한 정수값 또는 `취업 ^ Y * Z%`를 올림한 정수값을 `대기업` 스택에 넣는 연산을 수행합니다. 괄호에 해당하는 부분은 생략할 수 있습니다.
`압도적` 키워드를 사용한 경우 `취업 ^ Y * Z%`로 사용하지 않은 경우 `취업 * Y * Z%`로 계산합니다.
...
### `대학` 취업률 (`압도적`) `Y`위 (`Z`%)
`Y * Z%`를 올림한 정수값 개수 만큼의 요소를 뒤에서부터 전부 더한 값을 스택에 넣습니다. `압도적` 키워드를 사용한 경우 결과값을 `중소기업`스택에 넣고, 사용하지 않은 경우 `대기업` 스택에 넣습니다. `대학` 리터럴이 `대학원`인 경우 결과값의 부호는 음수입니다. `Z`%가 사용되지 않은 경우, `Z`%는 100%로 평가됩니다.
```
SKY보다 KOREATECH
SKY보다 KOREATECH
<중소기업 스택=[ 1, 1 ]| 공시>
